...
# READ IN THE DATA ----
soil.df <- read_csv("Bill/Untitled/soils_all_years_raw_wide.csv")
...
library(GGally)
corr.df <- soil.df %>% 
  select(-c(date, crop_year, year, month, season, block_no, block_ns, treatment, 
            plot, season_orig, subplot_point, soils_phb, soils_b_ppm, soils_cu_ppm)) %>% 
  select(-ends_with("_base_satpct"))

# install.packages("ggcorrplot")

ggcorr(corr.df,   palette = "RdYlGn", label = TRUE) +coord_flip()
